"""Traceable evidence source of truth built from parsed resume sections."""

from collections import Counter
import re

from app.models.analysis import FactBank
from app.services.analysis.interfaces import FactBankBuilderInterface
...
from app.services.parsing.resume_entity_parser import section_block, extract_projects
...
class FactBankBuilder(FactBankBuilderInterface):
    ...
    def build(self, sections: dict[str, str]) -> FactBank:
        ...
        for section, (source, source_type) in SECTION_SOURCES.items():
            ...
            real_delivery = bool(re.search(
                ...
                text,
            )) if source_type in {"freela", "open_source", "project"} else None
            for technology in technologies:
                ...
                evidence_items.append({"item": technology, "source": source, "source_type": source_type,
...
def build_fact_bank(sections: dict[str, str]) -> FactBank:
    return FactBankBuilder().build(sections)


def summarize_sources(fact_bank: FactBank) -> dict[str, int]:
    return FactBankBuilder().summarize_sources(fact_bank)
